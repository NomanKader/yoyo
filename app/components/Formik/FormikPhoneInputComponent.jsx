...
const FormikPhoneInputComponent = memo(
  ({label, value, onChangeText, formikProps, formikKey}) => {
    const [selectedCountryCode, setSelectedCountryCode] = useState('+66'); // Default to US code
    ...
    const renderCountryCodeItem = useCallback(
      ({item}) => (
        <TouchableOpacity
    ...
    const handlePhoneNumberChange = text => {
      setPhoneNumber(text);
      const combinedValue = `${selectedCountryCode}${text}`;
      formikProps.setFieldValue(formikKey, combinedValue);
    };
